...
const MODE2_DEPLOY_SUPPORTED=false;
...
function lockUi(locked){
  state.setupLocked=locked;$('normalizedLength').disabled=locked;$('windowStride').disabled=locked;$('labelEntry').disabled=locked;$('addLabelBtn').disabled=locked;$('removeLabelBtn').disabled=locked;$('lockSetupBtn').disabled=locked;
  $('recordLabel').disabled=!locked;$('armRecordBtn').disabled=!(locked&&noodleBLE.connected);$('stopRecordBtn').disabled=true;$('trainBtn').disabled=!(locked&&state.targets.length);
}
...
$('lockSetupBtn').addEventListener('click',()=>{
  ...
  state.N=N;lockUi(true);refreshDataset();
...
$('datasetFile').addEventListener('change',async e=>{try{const file=e.target.files[0];if(!file)return;if((state.targets.length||state.setupLocked)&&!confirm('Replace the current dataset/setup?'))return;const arrays=await NAI.loadNpz(file);const ds=NAI.parseDatasetArrays(arrays);resetDataset(false);state.N=ds.N;$('normalizedLength').value=String(ds.N);state.labels=[...ds.labels];state.samples=ds.Xrows.map(r=>Float32Array.from(r));state.targets=Array.from(ds.y,Number);state.rawSamples=ds.rawRows?ds.rawRows.map(r=>({data:Float32Array.from(r.data),length:r.length})):[];state.rawLengths=arrays.raw_lengths?Array.from(arrays.raw_lengths.data,Number):Array(state.targets.length).fill(ds.N);state.durationsMs=arrays.durations_ms?Array.from(arrays.durations_ms.data,Number):Array(state.targets.length).fill(0);refreshLabels();lockUi(true);refreshDataset();$('recordProgress').textContent=`Loaded ${state.targets.length} windows from ${file.name}.`;log(`Loaded ${file.name}: ${state.targets.length} windows; raw windows ${state.rawSamples.length===state.targets.length?'available':'not available'}.`);}catch(err){alert(err.message);log(`Dataset load ERROR: ${err.message}`);}finally{e.target.value='';}});
...
$('deployBtn').addEventListener('click',async()=>{if(!state.pkg)return;try{$('deployBtn').disabled=true;$('trainingModeBtn').disabled=true;$('inferenceModeBtn').disabled=true;const chunk=Number($('chunkSize').value);await noodleBLE.deployPackage(state.pkg.files,{chunkSize:chunk,onProgress:p=>{const prog=$('deployProgress');prog.max=Math.max(1,p.total);prog.value=p.sent;const percent=p.total?Math.round(100*p.sent/p.total):0;const msg={begin:'Starting transactional deployment…','file-begin':`Preparing ${p.file}…`,sending:`${p.file}: ${percent}% total`,commit:'Files verified; validating Noodle model…',done:'MODEL_OK — model activated and ready.',error:`Deployment failed: ${p.error||'unknown error'}`}[p.stage]||p.stage;$('deployText').textContent=msg;}});log('Deployment complete: MODEL_OK.');}catch(e){alert(e.message);log(`Deployment ERROR: ${e.message}`);}finally{$('deployBtn').disabled=!MODE2_DEPLOY_SUPPORTED||!(noodleBLE.connected&&state.pkg);$('trainingModeBtn').disabled=!noodleBLE.connected;$('inferenceModeBtn').disabled=!noodleBLE.connected;}});
...
noodleBLE.addEventListener('status',e=>{const text=e.detail.text;if(e.detail.notify)log(`Device: ${text}`);if(text==='MODE:T'){state.deviceMode='T';$('modeStatus').textContent='Current mode: TRAINING';$('deviceStatus').textContent='Training mode';}else if(text==='MODE:I'){state.deviceMode='I';$('modeStatus').textContent='Current mode: INFERENCE';$('deviceStatus').textContent='Inference mode';}else if(text==='MODE2:STREAM'){$('modeStatus').textContent='Current mode: MODE 2 STREAMING';$('deviceStatus').textContent='Mode 2 streaming';}else if(text==='MODEL_OK'){switchTab('deploy');$('deployText').textContent='Model verified, activated, and ready.';}else if(text.startsWith('P:')){const p=text.split(':');if(p.length>=3){const idx=Number(p[1]),conf=Number(p[2]);const label=state.labels[idx]??String(idx);$('predictionLabel').textContent=label;$('predictionConfidence').textContent=`Confidence ${pct(conf)}`;$('deviceStatus').textContent=`Inference: ${label} (${pct(conf)})`;}}});
